...
function App() {
  ...
  useEffect(() => {
    ...
    fetch(`${window.location.protocol}//${window.location.hostname}:5000/api/stores`)
  ...
  useEffect(() => {
    ...
    if (selectedStore) {
      fetch(`${window.location.protocol}//${window.location.hostname}:5000/api/products/${selectedStore}`)
        .then((response) => response.json())
